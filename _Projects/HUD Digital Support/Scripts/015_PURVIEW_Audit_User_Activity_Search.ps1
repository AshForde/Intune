...
# Requirements
#Requires -Modules Microsoft.Graph.Authentication, Microsoft.Graph.Beta.Security
...
while ($retryCount -lt $maxRetries -and $searchStatus -ne "succeeded") {
    ...
    $searchResult = Get-MgBetaSecurityAuditLogQuery -AuditLogQueryId $SearchID
...
if ($searchStatus -eq "succeeded") {
    ...
    $results = Get-MgBetaSecurityAuditLogQue     -AuditLogQueryId $SearchID
